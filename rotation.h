#ifndef RANK_FHE_ROTATION_H
#define RANK_FHE_ROTATION_H
...
template <int N> class RotationComposer {
  ...
  public:
    ...
    // 新增旋转组合方法
    Ciphertext<DCRTPoly> composeRotations(const Ciphertext<DCRTPoly>& input, const std::vector<int>& rotations) {
        Ciphertext<DCRTPoly> result = input->Clone();
        for (int rot : rotations) {
            result = rotate(result, rot);
        }
        return result;
    }
};
...
// 旋转有效性检查扩展
inline bool isRotationValid(int rotation, int N) {
    return std::abs(rotation) < N / 2;
}
#endif // RANK_FHE_ROTATION_H